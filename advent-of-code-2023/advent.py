...
def binary_search(predicate, low, high) -> int | None:
...
def bool_list_to_decimal(lst) -> int:
...
def decimal_to_bool_list(n) -> list[int]:
    return [int(digit) for digit in bin(n)[2:]]


def file_to_lines(fname) -> list[str]:
...
def parse(
    day: int, parser: Callable[[str], Any] = str, sep: str = '\n', print_lines: int | None = None
) -> tuple[Any, ...]:
